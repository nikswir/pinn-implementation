...
def u_exact(x: Any, y: Any, t: Any, n_terms: int = 4) -> np.ndarray:
    """Vectorized analytic solution over point arrays (broadcast together)."""
    x, y, t = np.broadcast_arrays(
        np.asarray(x, float),
        np.asarray(y, float),
        np.asarray(t, float),
    )
    out: np.ndarray = (-np.exp(-t) + t**2 / 2.0 - t + 1.0) * np.sin(y)
    for i in range(n_terms):
        n = 2 * i + 1
        lam = (math.pi * n) ** 2 + 1.0
        coeff = -4.0 / (((math.pi * n) ** 2) * (lam**3))
        time_part = (
            (t * lam) ** 2 - 2.0 * t * lam + 2.0 * (1.0 - np.exp(-lam * t))
        )
